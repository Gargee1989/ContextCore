(() => {
	...
	function contextFor(selection, sourceText = pageText) {
		...
		return sourceText.slice(start, Math.min(sourceText.length, index + selected.length + 700)).trim();
	}
	...
	async function save(context) {
		...
		if (!saved.some((item) => item.word === selectedText && item.document === currentDocument)) {
			saved.unshift({ word: selectedText, context, definition: currentDefinition, document: currentDocument, savedAt: Date.now() });
		}
