(() => {
	...
	function createCard(rect, context) {
		...
		shadowRoot.innerHTML = `
			...
			<section data-contentcore="lookup-card">
				<div class="cc-header">
		...
		const lookupCard = shadowRoot.querySelector('[data-contentcore="lookup-card"]');
		shadowRoot.querySelector(".cc-word").textContent = selectedText;
		...
		lookupHost.addEventListener("mouseup", (event) => event.stopPropagation());
		shadowRoot.querySelector("[data-cc-close]").addEventListener("click", removeCard);
		shadowRoot.querySelector("[data-cc-explain]").addEventListener("click", explainSelection);
		shadowRoot.querySelector("[data-cc-save]").addEventListener("click", saveSelection);
	}
	...
	function setStatus(message, isError = false) {
		const status = shadowRoot?.querySelector(".cc-status");
		if (status) {
	...
	async function explainSelection() {
		...
		const button = shadowRoot.querySelector("[data-cc-explain]");
	...
	function renderResult(definition, fromCache = false) {
		...
		shadowRoot.querySelector(".cc-status").textContent = fromCache ? "From cache" : "✓ Done";
		shadowRoot.querySelector(".cc-result").textContent = definition;
		shadowRoot.querySelector(".cc-result").hidden = false;
		shadowRoot.querySelector("[data-cc-explain]").hidden = true;
		shadowRoot.querySelector("[data-cc-save]").hidden = false;
	}
